...
import com.weirddev.testme.intellij.generator.MyTestMeGenerator;
import com.weirddev.testme.intellij.generator.TestMeGenerator;
import com.weirddev.testme.intellij.template.EmptyGroovyTemplateDescriptor;
 ...
 */
public class MyCreateTestMeAction extends CreateTestAction {
    private static final Logger LOG = Logger.getInstance(MyCreateTestMeAction.class.getName());
    private static final String CREATE_TEST_IN_THE_SAME_ROOT = "create.test.in.the.same.root";
    private final MyTestMeGenerator testMeGenerator;
    ...
    public MyCreateTestMeAction() {
        this(new MyTestMeGenerator(), new TargetDirectoryLocator(), new GeneratedClassNameResolver());
    }

    MyCreateTestMeAction(MyTestMeGenerator testMeGenerator, TargetDirectoryLocator targetDirectoryLocator, GeneratedClassNameResolver generatedClassNameResolver) {
    ...
    public void invoke(@NotNull final Project project, Editor editor, @NotNull PsiElement element) throws IncorrectOperationException {
        ...
        if (testRootUrls.isEmpty() && computeSuitableTestRootUrls(testModule).isEmpty()) {
            ...
            if (!propertiesComponent.getBoolean(CREATE_TEST_IN_THE_SAME_ROOT, false)) {
                if (Messages.showOkCancelDialog(project, "Create test in the same source root?", "No Test Roots Found", "Ok", "Cancel", Messages.getQuestionIcon()) !=
                        Messages.OK) {
                    return;
                }
        ...
        String testClassName = GeneratedClassNameResolver.composeTestClassName(srcClass);
        final Module finalTestModule = testModule;
        CommandProcessor.getInstance().executeCommand(project, () -> testMeGenerator.generateTest(
                new FileTemplateContext(
                        null, Language.Groovy, project, testClassName, srcPackage, srcModule, finalTestModule, targetDirectory, srcClass,
...
//    }
    ...
    /**
     * @see CreateTestAction#getContainingClass(PsiElement)
